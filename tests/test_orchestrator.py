...
class OrchestratorTests(unittest.TestCase):
    ...
    def make_node(self, node_id: str, payload: dict, dependencies=None):
        ...
        return {
            ...
            'owner_skill': adapter['owner_skill'],
            'project_id': adapter['project_id'],
            'workflow': adapter['workflow'],
            'request_file': adapter.get('request_file'),
            'request_file_pattern': adapter.get('request_file_pattern'),
            'result': adapter.get('result'),
            'result_pattern': adapter.get('result_pattern'),
            'artifact_pattern': adapter.get('artifact_pattern'),
            'remote_trigger': adapter.get('remote_trigger'),
            'dependencies': dependencies or [],
            'operation': dispatcher['default_operation'],
            'input_fingerprint': 'b' * 64,
            'dispatcher': dispatcher,
            'invocation': {
                'mode': mode,
                'path': path,
                'base_ref': dispatcher.get('base_ref'),
                'branch_mode': dispatcher.get('branch_mode'),
                'target_branch': dispatcher.get('target_branch'),
                'payload': payload,
            },
        }
    ...
    def make_request(self, nodes, order, receipts=None):
        ...
        return {
            ...
            'registry_fingerprint': self.registry_fp,
            'transport_mode': 'github_app',
            'workflow_state': 'execution_active',
    ...
    def test_validator_and_dependency_wave(self):
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            calls = []
            module.ensure_ephemeral_request = lambda repo, base, branch, path, payload, message: (calls.append((repo, path)) or ('invoked', 'd' * 40))
            old_argv = sys.argv
            try:
            ...
            statuses = {item['id']: item['status'] for item in transport['nodes']}
            self.assertEqual(statuses['seochecker'], 'invoked')
            self.assertEqual(statuses['checklist'], 'waiting_dependencies')
    ...
    def test_append_existing_branch_is_preserved(self):
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            module = load_dispatch_module()
            seen = {}
            def append(repo, branch, path, payload, message):
                seen.update(repo=repo, branch=branch, path=path)
                return 'invoked', 'e' * 40
            module.ensure_append_request = append
            old_argv = sys.argv
            try:
                sys.argv = ['dispatch_nodes.py', str(request_path), str(td / 'transport.json')]
                self.assertEqual(module.main(), 0)
    ...
    def test_exact_duplicate_is_ignored(self):
        module = load_dispatch_module()
        payload = {'request_id': 'seo-123', 'url': 'https://example.com'}
        encoded = base64.b64encode((json.dumps(payload) + '\n').encode()).decode()
        module.get_ref_sha = lambda repo, ref: 'a' * 40
        module.try_get_ref_sha = lambda repo, ref: 'a' * 40
        module.get_file = lambda repo, branch, path: {'type': 'file', 'encoding': 'base64', 'content': encoded}
        module.latest_commit_for_path = lambda repo, branch, path: 'f' * 40
        module.create_ref = lambda *args: self.fail('create_ref should not run for duplicate')
        module.put_file = lambda *args: self.fail('put_file should not run for duplicate')
        status, sha = module.ensure_ephemeral_request('Yolol100/seochecker', 'main', 'runtime/test', 'requests/audit.json', payload, 'msg')
    ...
    def test_cleanup_wordpress_branch_requires_controller_verified_current_head(self):
        ...
        spec = importlib.util.spec_from_file_location('cleanup_runtime_branches_under_test', path)
        module = importlib.util.module_from_spec(spec)
        assert spec and spec.loader
        spec.loader.exec_module(module)
        with tempfile.TemporaryDirectory() as raw:
            td = Path(raw)
            branch = 'runtime/wf-abcdef123456-g1-wordpressconnector-cccccccc'
            plan = {'workflow_id': 'WF-ABCDEF123456', 'nodes': [{'repository': 'Yolol100/wordpressconnector', 'branch': branch, 'head_sha': 'a' * 40}], 'temporary_branches': [{'repository': 'Yolol100/wordpressconnector', 'branch': branch, 'cleanup_required': True, 'cleanup_requires_controller_verified_head': True}]}
            plan_path = td / 'transport.json'
            plan_path.write_text(json.dumps(plan), encoding='utf-8')
            old_argv = sys.argv
            try:
                sys.argv = ['cleanup_runtime_branches.py', str(plan_path), '--confirm-workflow-id', 'WF-ABCDEF123456']
                with self.assertRaises(SystemExit) as ctx:
                    module.main()
                self.assertIn('missing controller-verified current head', str(ctx.exception))
            finally:
                sys.argv = old_argv
            verified_path = td / 'verified-heads.json'
            verified_path.write_text(json.dumps({f'Yolol100/wordpressconnector:{branch}': 'b' * 40}), encoding='utf-8')
            module.ref_sha = lambda repo, current_branch: 'b' * 40
            deleted = []
            module.gh = lambda method, endpoint: deleted.append((method, endpoint)) or {}
            try:
                sys.argv = ['cleanup_runtime_branches.py', str(plan_path), '--confirm-workflow-id', 'WF-ABCDEF123456', '--verified-heads', str(verified_path)]
                self.assertEqual(module.main(), 0)
            finally:
                sys.argv = old_argv
    ...
    def test_runtime_scripts_use_current_github_rest_api_version(self):
        for name in ('dispatch_nodes.py', 'cleanup_runtime_branches.py'):
            text = (ROOT / 'scripts' / name).read_text(encoding='utf-8')
            self.assertIn('X-GitHub-Api-Version: 2026-03-10', text)
            self.assertNotIn('X-GitHub-Api-Version: 2022-11-28', text)
