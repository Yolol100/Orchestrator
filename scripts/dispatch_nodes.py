...
def ensure_pull_request_request(
    repo: str,
    base_ref: str,
    branch: str,
    path: str,
...
def main() -> int:
    ...
    for node_id in request['execution_order']:
        ...
        _ACTIVE_TOKEN = (
            os.environ.get('GH_TOKEN_PULL_REQUESTS') if mode == 'pull_request'
            else os.environ.get('GH_TOKEN_CONTENTS')
        ...
        if branch_mode == 'ephemeral_runtime_branch' and mode == 'request_file':
            ...
            status, head_sha, pr_number, pr_url = ensure_pull_request_request(
                repo,
                base_ref,
                branch,
                path,
                payload,
                message,
                f"Runtime request {request['workflow_id']} g{request['generation']}",
                'Controller-approved Webactueel runtime request. Do not merge this PR; the guarded runtime workflow reads the request from the PR head and writes its result back to the same branch.',
            )
            temporary_branches.append({
                ...
                'cleanup_requires_controller_verified_head': True,
            })
        else:
