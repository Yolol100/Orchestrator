...
def main() -> int:
    ...
    for key in ('workflow_id', 'generation', 'idempotency_key', 'registry_fingerprint'):
        if transport.get(key) != request.get(key):
            errors.append(f'mismatch:{key}')
    if transport.get('request_sha256') != hashlib.sha256(request_bytes).hexdigest():
        errors.append('request_sha256')
    if transport.get('return_to') != 'webactueel-workflow':
        errors.append('return_to')
    if transport.get('next_action') != 'controller_readback_and_resume':
    ...
    if {node.get('id') for node in transport_nodes} != set(request_nodes):
        errors.append('node_set')
    accepted = set((request.get('dependency_receipts') or {}).keys())
    for item in transport_nodes:
        node_id = item.get('id')
        source = request_nodes.get(node_id)
        if not source:
            continue
        missing = [dep for dep in source.get('dependencies', []) if dep not in accepted]
        status = item.get('status')
        if missing:
            if status != 'waiting_dependencies' or sorted(item.get('missing_dependencies') or []) != sorted(missing):
                errors.append(f'dependency_gate:{node_id}')
            continue
        if status not in {'invoked', 'duplicate_ignored'}:
            errors.append(f'invocation_status:{node_id}')
            continue
    ...
    for entry in temp:
        if entry.get('cleanup_required') is not True or not str(entry.get('branch') or '').startswith('runtime/'):
            errors.append('temporary_branch_contract')
